	...
	<section class="py-5">
		<div class="container border px-5 py-4">
			...
			<div class="my-4">
				<div class="mapinfo">
					<div class="product-info">
						<div class="user">
							<span class="user-id"> <a
								href="${pageContext.request.contextPath}/product/search?q=user_id&value=${p.user_id}"
								title="판매자의 다른 상품보기">${p.member.user_nickname}</a>
							</span> (${p.member.user_nickname},
							<c:choose>
								<c:when test="${p.member.user_scorecount != 0}">
									<i class="fas fa-star"></i>
									<fmt:formatNumber
										value="${p.member.user_score/p.member.user_scorecount}"
										pattern="#.##점" />,
							...
							<nav style="-bs-breadcrumb-divider: '&gt;'"
								...
								<ol class="breadcrumb">
									<li class="breadcrumb-item active"><a
										...
										title="해당 대분류로 검색">${p.c1.category_name}</a></li>
						...
						<div class="product-data my-2">
							...
							| 댓글 ${p.replys.size()} | <a id="reportbtn"
						...
						<c:choose>
							...
							<c:otherwise>
								<c:choose>
									<c:when test="${p.product_result != 2}">
										<label class="like"> <input type="checkbox" id="wish" />
											<div class="hearth"></div>
										</label>
										<!-- 신고하기 -->
										<div class="modal fade" id="rb" tabindex="-1"
												...
												<div class="modal-content">
													...
													<div class="modal-body">
														<form id="report_form" method="post"
															...
															<div class="mb-3">
																<label for="recipient-name" class="col-form-label">신고
																	사유</label> <input type="text" class="form-control  w-100"
																	id="recipient-name" name="report_title">
															</div>
															<div class="mb-3">
																<label for="message-text" class="col-form-label">세부
																	내용을 적어주세요</label>
			...
			<div class="replys" id="reply-add-form">
				<!-- 댓글 등록 폼 -->
				<form action="${pageContext.request.contextPath}/reply/add"
					method="post" class="add-reply-form">
					<div class="input-group add-reply">
						<textarea name="reply_content" class="form-control" rows="4"
							aria-describedby="reply" required></textarea>
						<button class="btn btn-outline-secondary reply-btn" type="submit">댓글
							등록</button>
					</div>
				...
				<ul class="list-group replys">
					<c:forEach var="reply" items="${p.replys}">
						<c:set var="status"
							value="${reply.reply_content==null?'deleted':p.user_id==reply.user_id?'seller':'other'}" />
						<c:set var="className"
							value="${reply.reply_content==null?'deleted':p.user_id==reply.user_id?'primary':'info'}" />
						<%-- 한개의 댓글 내용 --%>
						<c:choose>
							<%-- 댓글이 정상상태인 경우 --%>
							<c:when test="${status!='deleted'}">
								<li
									...
									<div class="reply-content" id="reply-${reply.reply_num}">
										${reply.reply_content}</div>
									<div class="reply-writer">
										<c:if
											test="${p.user_id==sessionScope.user_id && p.product_result != 2}">
											<a name="reply-user_id" data-toggle="popover"
									...
									<div class="reply-menu mt-3">
										<c:if test="${sessionScope.user_id!=null}">
											<a onclick="toggleReply(`${reply.reply_num}`)" title="댓글"><i
												class="fas fa-reply"></i> 댓글</a>
										</c:if>
										<c:if test="${reply.user_id==sessionScope.user_id}">
											<span> | </span>
											<a onclick="editReply(`${reply.reply_num}`)" title="수정"><i
												class="fas fa-edit"></i> 수정</a>
											<span> | </span>
											<a onclick="deleteReply(`${reply.reply_num}`)" title="삭제"><i
								...
								<%-- 댓글 내용 끝 & 댓글의 댓글 추가 폼 --%>
								<form action="${pageContext.request.contextPath}/reply/add"
									method="post" class="add-child-reply-form"
									id="form-${reply.reply_num}" style="display: none">
									<div class="input-group">
										<textarea name="reply_content" class="form-control" rows="4"
											aria-describedby="reply" required></textarea>
										<button class="btn btn-outline-secondary child-reply-btn"
											type="submit">댓글 등록</button>
									</div>
									<input type="hidden" name="user_id"
										value="${sessionScope.user_id}" /> <input type="hidden"
										name="product_num" value="${p.product_num}" /> <input
										type="hidden" name="parent_reply_num"
							...
							<c:forEach var="c_reply" items="${reply.child_reply}">
								...
								<c:choose>
									<c:when test="${child_status!='deleted'}">
										<li
											class="list-group-item list-group-item-${child_className} ${child_status} child-reply">
											<div id="reply-${c_reply.reply_num}"
												class="child reply-content">${c_reply.reply_content}</div>
											...
											</div> <%-- 댓글 쓴 사람==로그인 한 사람 --%> <c:if
												test="${c_reply.user_id==sessionScope.user_id}">
												<div class="child reply-menu mt-3">
													<a onclick="editReply(`${c_reply.reply_num}`)" title="수정"><i
														class="fas fa-edit"></i> 수정</a> <span> | </span> <a
														onclick="deleteReply(`${c_reply.reply_num}`)" title="삭제"><i
														class="fas fa-trash"></i> 삭제</a>
												</div>
											</c:if>
										</li>
	...
	function searchAddressToCoordinate(address) {
	    naver.maps.Service.geocode({
	        ...
	        infoWindow.setContent([
	            '<div style="padding:10px;max-width:200px;line-height:150%;">',
